...
class Pedidos(models.Model):
    ...
    data_entrega = models.DateField(null=False, blank=False, db_index=True)
    horario_pedido = models.TimeField(default=datetime.now().time(), null=False, blank=True)
    ordem_compra = models.CharField(max_length=50, null=True, blank=True, db_index=True)
    ...
    id_contato = models.ForeignKey(Contatos, null=True, blank=True, on_delete=models.PROTECT)
...
class ItensPedido(models.Model):
    ...
    custo_total = models.DecimalField(default=0, blank=True, max_digits=10, decimal_places=2)